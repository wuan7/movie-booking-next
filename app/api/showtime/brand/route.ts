...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!movieId || !showDate || !brandId) {
      return NextResponse.json({ message: "Missing movieId or showDate" }, { status: 400 });
    }
    ...
    const formattedNow = format(nowInTimeZone, 'yyyy-MM-dd HH:mm:ssXXX', { timeZone: 'Asia/Ho_Chi_Minh' });
    
    // Chuyển đổi showDate thành thời gian UTC vào cuối ngày (23:59:59.999)
    const endOfDayInUTC = new Date(new Date(showDate).setHours(23, 59, 59, 999)); 
